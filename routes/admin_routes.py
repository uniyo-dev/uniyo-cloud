...
def change_password():
    ...
    db.execute("UPDATE admins SET password_hash = ?, must_change_password = 0 WHERE id = ?", (new_hash, session['admin_id']))

    db.execute("INSERT INTO audit_logs (admin_id, action, target_table, target_id, details, created_at) VALUES (?, 'CHANGE_PASSWORD', 'admins', ?, 'Admin changed password', ?)", (session['admin_id'], session['admin_id'], datetime.now().isoformat()))
...
def issue_certificate():
    ...
    db.execute("INSERT INTO certificates (student_id, certificate_type, rank, month_year, certificate_number, verification_token, title, reason, issue_date, issued_by, full_name, university, stream, sex) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (student_id, certificate_type, rank, month_year, certificate_number, verification_token, title, reason, datetime.now().isoformat(), session['admin_id'], student.get('full_name', ''), student.get('university', ''), student.get('stream', ''), student.get('sex', '')))

    db.execute("INSERT INTO notifications (student_id, message, type, created_at) VALUES (?, 'You have received a new certificate!', 'certificate', ?)", (student_id, datetime.now().isoformat()))
...
def api_certificate_image(certificate_id):
    """Serve full certificate as PDF for admin viewing"""
    ...
    pdf_path = CERTIFICATES_DIR / f"{cert_id}.pdf"
    ...
    # Use ReportLab (Professional PDF)
