// ============================================
// UNIYO LMS - ADVANCED THEME MANAGER v3
// Features: No flicker, instant, persistent, smooth
// Integrates: master_theme.js + admin_theme.js features
// ============================================
...
const ThemeManager = {
    ...
    apply(theme, instant = false) {
        ...
        if (theme === 'light') {
            ...
            // Apply comprehensive light mode styling (from master_theme.js)
            if (instant || document.readyState === 'complete') {
                this.applyLightModeStyles();
            } else {
                window.addEventListener('load', () => {
                    setTimeout(() => this.applyLightModeStyles(), 100);
                });
            }
        } else {
            ...
            // Remove inline light mode styles (from master_theme.js)
            if (instant || document.readyState === 'complete') {
                this.removeLightModeStyles();
            } else {
                window.addEventListener('load', () => {
                    setTimeout(() => this.removeLightModeStyles(), 100);
                });
            }
        }
    ...
    // Comprehensive light mode styling (integrated from master_theme.js)
    applyLightModeStyles() {
        // Apply light mode to all major elements
        ...
        // Highlight strong tags in boxes
        ...
        document.querySelectorAll('svg line[stroke="#1a2332"]').forEach(el => {
            el.setAttribute('stroke', '#CBD5E1');
        });
    },

    // Remove light mode styles (restore dark mode)
    ...
    updateButtons(theme) {
        ...
        // Also update admin theme button (from admin_theme.js)
        const adminBtn = document.getElementById('adminThemeBtn');
        if (adminBtn) {
            adminBtn.textContent = theme === 'light' ? '☀️ Light' : '🌙 Dark';
        }
    },
